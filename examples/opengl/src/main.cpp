...
//#include "null-render/graphic/brushes/filter-brush.h"
//#include "null-render/graphic/brushes/sdf-text/sdf-text.h"
...
void draw_example(const std::shared_ptr<null::render::c_draw_list>& draw_list, const std::string_view& name, const float& y) {
	...
	//draw_list->add_command(null::render::c_matrix_command::instance(vec2_t<float>(100, 100)));
	...
	std::shared_ptr<null::render::c_filter_brush> filter_brsuh{ };
	{
		...
		filter->add_stop(color_t<int>(140, 140, 255, 120), 0.f);
		filter->add_stop(color_t<int>(140, 140, 255, 180), 0.5f);
		filter->add_stop(color_t<int>(255, 140, 140, 180), 0.5f);
		filter->add_stop(color_t<int>(255, 140, 140), 1.f);
		filter_brsuh = null::render::c_filter_brush::instance(filter);*/
	...
	*/
	...
	null::render::begin_frame(); {
		...
		std::call_once(once, []() {
			...
			draw_example(compiled_draw_list, "brush", 80);
		...
		null::render::draw_list->add_command(compiled_draw_list);
		...
		null::render::draw_list->add_text(std::format("[ opengl ] fps: {:3.0f} {}", 1.f / std::chrono::duration<float>{ frame_counter.representation() }.count(), std::abs(std::sin(timemes.representation().count() * 0.000000001f))), { }, text_brush);
...
int main() {
	...
	try {
		...
		DwmEnableBlurBehindWindow(glfwGetWin32Window(window.glfw_window), &blur_behind);
		DeleteObject(region);*/
