...
void main_loop() {	
	...
	null::render::begin_frame(window); {
		...
		null::render::background.add_text(std::format("[ directx11 ] fps: {:3.0f}", 1.f / std::chrono::duration<float>{ frame_counter.representation() }.count()), { }, text_brush);
