...
FontGeometry::FontGeometry() : geometryScale(1), metrics(), preferredIdentifierType(GlyphIdentifierType::UNICODE_CODEPOINT), glyphs(&ownGlyphs), rangeStart(glyphs->size()), rangeEnd(glyphs->size()), loading_flags(), embolden(), oblique() { }
...
int FontGeometry::loadGlyphRange(msdfgen::FontHandle *font, double fontScale, unsigned rangeStart, unsigned rangeEnd, bool preprocessGeometry, bool enableKerning) {
    ...
    for (unsigned index = rangeStart; index < rangeEnd; ++index) {
        GlyphGeometry glyph;
        if (glyph.load(font, geometryScale, msdfgen::GlyphIndex(index), preprocessGeometry, loading_flags, embolden, oblique)) {
...
int FontGeometry::loadCharset(msdfgen::FontHandle *font, double fontScale, const Charset &charset, bool preprocessGeometry, bool enableKerning) {
    ...
    for (unicode_t cp : charset) {
        GlyphGeometry glyph;
        if (glyph.load(font, geometryScale, cp, preprocessGeometry, loading_flags, embolden, oblique)) {
            addGlyph((GlyphGeometry &&) glyph);
            ++loaded;
        }
    }
...
void FontGeometry::setFlags(uint32_t flags) {
    loading_flags = flags;
}

void FontGeometry::setEmbolden(bool _embolden) {
    embolden = _embolden;
}

void FontGeometry::setOblique(bool _oblique) {
    oblique = _oblique;
}
