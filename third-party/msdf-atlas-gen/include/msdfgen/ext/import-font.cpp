...
class FontHandle {
    ...
    friend bool loadGlyph(Shape &output, FontHandle *font, GlyphIndex glyphIndex, FontCoordinateScaling coordinateScaling, double *outAdvance, uint32_t glyph_flags, bool embolden, bool oblique);
    friend bool loadGlyph(Shape& output, FontHandle* font, unicode_t unicode, FontCoordinateScaling coordinateScaling, double* outAdvance, uint32_t glyph_flags, bool embolden, bool oblique);
...
bool loadGlyph(Shape &output, FontHandle *font, GlyphIndex glyphIndex, FontCoordinateScaling coordinateScaling, double *outAdvance, uint32_t glyph_flags, bool embolden, bool oblique) {
    if (!font)
        return false;
    FT_Error error = FT_Load_Glyph(font->face, glyphIndex.getIndex(), FT_LOAD_NO_SCALE | glyph_flags);
    if (error)
        return false;

    if(embolden) FT_GlyphSlot_Embolden(font->face->glyph);
    if(oblique) FT_GlyphSlot_Oblique(font->face->glyph);
...
bool loadGlyph(Shape &output, FontHandle *font, unicode_t unicode, FontCoordinateScaling coordinateScaling, double *outAdvance, uint32_t glyph_flags, bool embolden, bool oblique) {
    return loadGlyph(output, font, GlyphIndex(FT_Get_Char_Index(font->face, unicode)), coordinateScaling, outAdvance, glyph_flags, embolden, oblique);
}

bool loadGlyph(Shape &output, FontHandle *font, GlyphIndex glyphIndex, double *outAdvance, uint32_t glyph_flags, bool embolden, bool oblique) {
    return loadGlyph(output, font, glyphIndex, FONT_SCALING_LEGACY, outAdvance, glyph_flags, embolden, oblique);
}

bool loadGlyph(Shape &output, FontHandle *font, unicode_t unicode, double *outAdvance, uint32_t glyph_flags, bool embolden, bool oblique) {
    return loadGlyph(output, font, unicode, FONT_SCALING_LEGACY, outAdvance, glyph_flags, embolden, oblique);
}
