...
void main() {
    vec3 msdf = texture(sampler, frag_uv.st).rgb;
    float sdf = median(msdf.r, msdf.g, msdf.b);

    float inverted_threshold = 1.0 - threshold;
    float inner = px_range * (sdf - inverted_threshold) + 0.5 + out_bias;
    float outer = px_range * (sdf - inverted_threshold + outline_width_relative) + 0.5 + out_bias + outline_width_absolute;
