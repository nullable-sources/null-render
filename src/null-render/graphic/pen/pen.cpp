...
namespace ntl::render {
    std::shared_ptr<i_command> pen_t::around_convex_shape(const std::shared_ptr<c_geometry_command>& command) const {
        ...
        return brush->prepare_command(std::move(pen_command));
    }

    std::shared_ptr<i_command> pen_t::around_stroke(const std::shared_ptr<c_geometry_command>& command, const std::vector<backend::index_t>& outward_order, const std::vector<backend::index_t>& inward_order, bool closed) const {
        std::shared_ptr<c_geometry_command> pen_command = c_geometry_command::instance(command->geometry_buffer);
    ...
    void pen_t::make_geometry(std::shared_ptr<c_geometry_command>& pen_command, const std::shared_ptr<c_geometry_command>& command, backend::index_t previous, backend::index_t current, backend::index_t next, math::e_rotation rotation, bool is_last, backend::index_t first_vertex_offset) const {
        const backend::vertex_t& previous_vertex = pen_command->geometry_buffer->vertex_buffer[command->vertex_offset + previous];
        const backend::vertex_t& current_vertex = pen_command->geometry_buffer->vertex_buffer[command->vertex_offset + current];
        const backend::vertex_t& next_vertex = pen_command->geometry_buffer->vertex_buffer[command->vertex_offset + next];
