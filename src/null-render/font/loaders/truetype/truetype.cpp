...
namespace null::render {
    void c_truetype_loader::build(c_atlas* atlas) {
        ...
        for(auto [src, config] : std::views::zip(src_array, atlas->configs)) {
            ...
            src.pack_range = stbtt_pack_range{
        ...
        for(auto [src, config] : std::views::zip(src_array, atlas->configs)) {
            ...
            const vec2_t<float> font_off = config.glyph_config.offset + vec2_t(0.f, std::round(dst_font->ascent));
            ...
            for(int glyph_i : std::views::iota(0, src.glyphs_count)) {
                ...
                rect_t uvs(q.s0, q.t0, q.s1, q.t1);

                float scale = 1.0f;
                if(sdf) {
                    static constexpr vec2_t<float> xy_padding(sdf_padding);
