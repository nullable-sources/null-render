...
namespace null::render::shaders {
	class c_general_texture_shader : public i_shader {
	public:
		c_matrix4x4 matrix{ };
		c_vertex_element<renderer::c_opengl3::vertex_t, 2, opengl::e_float, false, offsetof(renderer::c_opengl3::vertex_t, pos)> position{ };
		c_vertex_element<renderer::c_opengl3::vertex_t, 2, opengl::e_float, false, offsetof(renderer::c_opengl3::vertex_t, uv)> uv{ };
	...
	public:
		void create() override {
			...
			fragment->create();
			fragment->compile(memory::resource_t{ "null-renderer:opengl:shaders:fragment-texture", "null-renderer:opengl:resources" }.load().locked_data);
			program->attach_shader(fragment.get());

			vertex = std::make_unique<c_vertex_shader_wrapper>();
			vertex->create();
			vertex->compile(memory::resource_t{ "null-renderer:opengl:shaders:vertex-texture", "null-renderer:opengl:resources" }.load().locked_data);
