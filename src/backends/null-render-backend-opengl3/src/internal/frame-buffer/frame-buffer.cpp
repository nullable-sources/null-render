...
namespace null::render::backend::opengl3 {
	...
	void c_frame_buffer::destroy() {
		if(flags & e_frame_buffer_flags::msaa) opengl::delete_textures(1, &fbo_attachment);
	...
	void c_frame_buffer::copy_from(const std::unique_ptr<i_frame_buffer>& another_frame_buffer) {
		...
		opengl::blit_framebuffer(0, 0, another_frame_buffer->size.x, another_frame_buffer->size.y, 0, 0, size.x, size.y, opengl::e_color_buffer_bit, opengl::e_nearest);
	}
}