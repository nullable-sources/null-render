    ...
    public:
        void build(c_atlas* atlas) override {
            ...
            for(build_src_t& src : src_array) {
                ...
                for(const std::uint16_t* src_range{ src.src_ranges }; src_range[0] && src_range[1]; src_range += 2) {
                    for(std::uint32_t codepoint : std::views::iota(src_range[0], src_range[1] + 1u)) {
                        if(dst.glyphs_set[codepoint >> 5] & 1u << (codepoint & 31)) continue;
            ...
            for(auto [src, config] :std::views::zip(src_array, atlas->configs)) {
                ...
                const bool sdf{ config.render_mode == e_font_render_mode::sdf };
                src.pack_range = {
                	sdf ? sdf_pixel_size : config.size_pixels,
                	0,
                	src.glyphs_list.data(),
                	(int)src.glyphs_list.size(),
                    src.packed_chars,
                	(std::uint8_t)(sdf ? 1 : config.oversample.x),
                	(std::uint8_t)(sdf ? 1 : config.oversample.y)
                };
            ...
            for(auto [src, config] : std::views::zip(src_array, atlas->configs)) {
                ...
                const vec2_t<float> font_off{ config.glyph_config.offset + vec2_t<float>{ 0.f, std::round(dst_font->ascent) } };
                ...
                for(int glyph_i : std::views::iota(0, src.glyphs_count)) {
                    ...
                    rect_t<float> corners{ rect_t<float>{ q.x0, q.y0, q.x1, q.y1 } + font_off };
                    rect_t<float> uvs{ q.s0, q.t0, q.s1, q.t1 };

                    float scale{ 1.0f };
                    if(sdf) {
                        const vec2_t<float> xy_padding{ sdf_padding };
                        const vec2_t<float> uv_padding{ xy_padding / atlas->texture.size };
