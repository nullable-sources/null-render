...
export module null.render:font.freetype_loader;
...
export namespace null::render {
    ...
    public:
        void build(c_atlas* atlas) override {
            ...
            for (std::tuple<build_src_t&, font_config_t&> tuple : std::views::zip(src_array, atlas->configs)) {
                build_src_t& src{ std::get<build_src_t&>(tuple) };
                font_config_t& config{ std::get<font_config_t&>(tuple) };
                ...
                if (config.rasterizer_flags & e_font_rasterizer_flags::no_hinting) src.freetype.flags |= FT_LOAD_NO_HINTING;
                if (config.rasterizer_flags & e_font_rasterizer_flags::no_auto_hint) src.freetype.flags |= FT_LOAD_NO_AUTOHINT;
                if (config.rasterizer_flags & e_font_rasterizer_flags::force_auto_hint) src.freetype.flags |= FT_LOAD_FORCE_AUTOHINT;
                if (config.rasterizer_flags & e_font_rasterizer_flags::light_hinting) src.freetype.flags |= FT_LOAD_TARGET_LIGHT;
                else if (config.rasterizer_flags & e_font_rasterizer_flags::mono_hinting) src.freetype.flags |= FT_LOAD_TARGET_MONO;
                else src.freetype.flags |= FT_LOAD_TARGET_NORMAL;

                switch (config.render_mode) {
                	case e_font_render_mode::monochrome: { src.freetype.render_mode = FT_RENDER_MODE_MONO; } break;
                	case e_font_render_mode::sdf: { utils::logger.log(utils::e_log_type::warning, "freetype font loader does not support sdf fonts yet."); }
            ...
            for (std::tuple<build_src_t&, font_config_t&> tuple : std::views::zip(src_array, atlas->configs)) {
                ...
                for (const int& i : std::views::iota((size_t)0, src.glyphs_list.size())) {
                    futures.push_back(std::async(std::launch::async, [&](build_src_t* src, src_glyph_t* glyph, int index) {
                        ...
                        if (src->freetype.rasterizer_flags & e_font_rasterizer_flags::bold) FT_GlyphSlot_Embolden(src->freetype.face->glyph);
                        if (src->freetype.rasterizer_flags & e_font_rasterizer_flags::oblique) FT_GlyphSlot_Oblique(src->freetype.face->glyph);
            ...
            for (std::tuple<build_src_t&, font_config_t&> tuple : std::views::zip(src_array, atlas->configs)) {
                ...
                for (const int& i : std::views::iota(0, src.glyphs_count)) {
                    ...
                    if (glyph.glyph.corners.min + atlas->texture.glyph_padding > vec2_t<int>{ pack_rect.w, pack_rect.h }) {
                        utils::logger.log(utils::e_log_type::error, "info.size + texture.glyph_padding > vec2_t{ pack_rect.w, pack_rect.h }."); continue;
                    }

                    vec2_t<int> t{ pack_rect.x, pack_rect.y };
                    ...
                    dst_font->add_glyph(&config, (std::uint16_t)glyph.glyph.codepoint, rect_t{ glyph.glyph.texture_coordinates.min + config.glyph_config.offset + vec2_t<float>{ 0.f, round(dst_font->ascent) } } + rect_t<float>{ vec2_t<float>{ }, glyph.glyph.corners.min }, rect_t<float>{ t, t + glyph.glyph.corners.min } / atlas->texture.size, glyph.glyph.advance_x);
                }
            }
