		...
		for(const stroke_t::segment_t& segment : stroke.build_segments(points)) {
			...
			if(stroke.line_cap != e_line_cap::joint && (segment.is_first || segment.is_last)) {
				...
				if(stroke.line_join == e_line_join::bevel) {
					...
					const vec2_t<float> to_previous_vertex{ *segment.begin_edge->point + math::rotate_90_degrees<float>(segment.begin_edge->to_previous_direction, rotation) * half_thickness };
					const vec2_t<float> to_next_vertex{ *segment.begin_edge->point + math::rotate_90_degrees<float>(segment.begin_edge->to_next_direction, rotation) * half_thickness };

					command->vertex_count += 3;
					backend::mesh->geometry_buffer
						.add_vertex({ floor_pos ? math::floor(miter_vertex) : miter_vertex, { }, brush.color })
						.add_vertex({ floor_pos ? math::floor(to_previous_vertex) : to_previous_vertex, { }, brush.color })
						.add_vertex({ floor_pos ? math::floor(to_next_vertex) : to_next_vertex, { }, brush.color });
					...
					const vec2_t<float> outward_vertex{ *segment.begin_edge->point + math::rotate_90_degrees<float>(segment.begin_edge->to_next_direction, math::e_rotation::ccw) * half_thickness };
					const vec2_t<float> inward_vertex{ *segment.begin_edge->point + math::rotate_90_degrees<float>(segment.begin_edge->to_next_direction, math::e_rotation::cw) * half_thickness };
			...
			if(stroke.line_join == e_line_join::none) {
				const vec2_t<float> outward_vertex{ *segment.end_edge->point + math::rotate_90_degrees<float>(segment.begin_edge->to_next_direction, math::e_rotation::ccw) * half_thickness };
				const vec2_t<float> inward_vertex{ *segment.end_edge->point + math::rotate_90_degrees<float>(segment.begin_edge->to_next_direction, math::e_rotation::cw) * half_thickness };
