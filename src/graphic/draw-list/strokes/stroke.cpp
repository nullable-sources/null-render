...
namespace null::render {
	void stroke_t::segment_t::i_edge::from_points(const vec2_t<float>& previous_point, const vec2_t<float>& current_point, const vec2_t<float>& next_point, math::e_rotation normal_rotation) {
		...
		normal = math::vectors_bisector(to_next_direction, from_previous_direction, normal_rotation);

		miter_angle = angle_t<radians_t>{ math::angle_between(
			math::invert_vector_axis(to_next_direction, math::e_rotation::cw),
			math::invert_vector_axis(from_previous_direction, math::e_rotation::cw)
		) }.normalized() / 2.;
