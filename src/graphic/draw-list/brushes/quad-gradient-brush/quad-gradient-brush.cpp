...
namespace null::render {
	std::unique_ptr<commands::i_command> quad_gradient_brush_t::prepare_command(std::unique_ptr<commands::c_geometry>& command) const {
		...
		for(const impl::vertex_t& vertex : renderer->geometry_buffer.vertex_buffer | std::views::drop(command->vertex_offset) | std::views::take(command->vertex_count)) {
			min = math::min(vertex.pos, min);
			max = math::max(vertex.pos, max);
		}

		for(impl::vertex_t& vertex : renderer->geometry_buffer.vertex_buffer | std::views::drop(command->vertex_offset) | std::views::take(command->vertex_count)) {
